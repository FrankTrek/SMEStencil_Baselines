void stencil_3d_star_BrickBrick_1644_radius2(float *p0, float *p1, int ker_size_x, int ker_size_y, int ker_size_z, int sy, int sz, int dy, int dz, float *coef)
{
			...
			if(vec_size_y == 4 && vec_size_z == 4) {
				...
				for(int vx = 0; vx < ker_size_x; vx += 16) {
					...
					ov300 = svdup_f32_x(p32_all, 0); ov310 = svdup_f32_x(p32_all, 0); ov320 = svdup_f32_x(p32_all, 0); ov330 = svdup_f32_x(p32_all, 0); 
					{
						...
						temp000 = svld1(mask_x_0, &ptr_src[0 + (1*sz+1*16*4)]); 
						ov100 = svmla_x(p32_all, ov100, temp000, coef[4]); ov200 = svmla_x(p32_all, ov200, temp000, coef[3]); ov300 = svmla_x(p32_all, ov300, temp000, coef[2]); 
						temp010 = svld1(mask_x_0, &ptr_src[0 + (1*16+1*sz+1*16*4)]); 
						ov110 = svmla_x(p32_all, ov110, temp010, coef[4]); ov210 = svmla_x(p32_all, ov210, temp010, coef[3]); ov310 = svmla_x(p32_all, ov310, temp010, coef[2]); 
						temp020 = svld1(mask_x_0, &ptr_src[0 + (2*16+1*sz+1*16*4)]); 
						ov120 = svmla_x(p32_all, ov120, temp020, coef[4]); ov220 = svmla_x(p32_all, ov220, temp020, coef[3]); ov320 = svmla_x(p32_all, ov320, temp020, coef[2]); 
						temp030 = svld1(mask_x_0, &ptr_src[0 + (3*16+1*sz+1*16*4)]); 
						ov130 = svmla_x(p32_all, ov130, temp030, coef[4]); ov230 = svmla_x(p32_all, ov230, temp030, coef[3]); ov330 = svmla_x(p32_all, ov330, temp030, coef[2]); 
						//compute using grid[:, :, 4]
						temp000 = svld1(mask_x_0, &ptr_src[0 + (2*sz)]); 
						ov200 = svmla_x(p32_all, ov200, temp000, coef[4]); ov300 = svmla_x(p32_all, ov300, temp000, coef[3]); 
						temp010 = svld1(mask_x_0, &ptr_src[0 + (1*16+2*sz)]); 
						ov210 = svmla_x(p32_all, ov210, temp010, coef[4]); ov310 = svmla_x(p32_all, ov310, temp010, coef[3]); 
						temp020 = svld1(mask_x_0, &ptr_src[0 + (2*16+2*sz)]); 
						ov220 = svmla_x(p32_all, ov220, temp020, coef[4]); ov320 = svmla_x(p32_all, ov320, temp020, coef[3]); 
						temp030 = svld1(mask_x_0, &ptr_src[0 + (3*16+2*sz)]); 
						ov230 = svmla_x(p32_all, ov230, temp030, coef[4]); ov330 = svmla_x(p32_all, ov330, temp030, coef[3]); 
						//compute using grid[:, :, 5]
						temp000 = svld1(mask_x_0, &ptr_src[0 + (2*sz+1*16*4)]); 
						ov300 = svmla_x(p32_all, ov300, temp000, coef[4]); 
						temp010 = svld1(mask_x_0, &ptr_src[0 + (1*16+2*sz+1*16*4)]); 
						ov310 = svmla_x(p32_all, ov310, temp010, coef[4]); 
						temp020 = svld1(mask_x_0, &ptr_src[0 + (2*16+2*sz+1*16*4)]); 
						ov320 = svmla_x(p32_all, ov320, temp020, coef[4]); 
						temp030 = svld1(mask_x_0, &ptr_src[0 + (3*16+2*sz+1*16*4)]); 
						ov330 = svmla_x(p32_all, ov330, temp030, coef[4]); 
					}
					{
