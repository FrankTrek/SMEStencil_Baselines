void stencil_3d_box_BrickBrick_16164_radius1(float *p0, float *p1, int ker_size_x, int ker_size_y, int ker_size_z, int sy, int sz, int dy, int dz, float *coef)
{
	...
	int k_offset[] = {(-1*sz+3*16*4),0,(1*16*4),(2*16*4),(3*16*4),(1*sz)};
	int j_offset[] = {(-1*sy+3*16),0,(1*16),(2*16),(3*16),(1*sy),(1*sy+1*16),(1*sy+2*16),(1*sy+3*16),(2*sy),(2*sy+1*16),(2*sy+2*16),(2*sy+3*16),(3*sy),(3*sy+1*16),(3*sy+2*16),(3*sy+3*16),(4*sy)};
		...
		for(int vy = 0; vy < ker_size_y; vy += 16) {
			...
			for(int vx = 0; vx < ker_size_x; vx += 16) {
				...
				if(vec_size_y == 16 && vec_size_z == 4) {
					...
					svprfb_gather_offset(prefetch_mask, ptr_src + 2*16*4*4 + 4*sy + sz, index_vec, SV_PLDL1KEEP);


					for(int rz = 0; rz < 3; rz += 1) {
						float* coef_loc = coef + 3*3*rz;
						...
						mask_x_0 = svwhilelt_b32(0, ker_size_x - vx + 1 + 16); 
						mask_x_1 = svwhilelt_b32(16, ker_size_x - vx + 1 + 16); 
						mask_x_2 = svwhilelt_b32(32, ker_size_x - vx + 1 + 16); 
						mask_ext = svwhilelt_b32(0, 15); mask_ext = sveor_z(p32_all, mask_ext, p32_all); 
						mask_x_0 = svand_z(p32_all, mask_x_0, mask_ext); 
						for(int ry = 0; ry < 3; ry += 1) {
							float* alpha_loc = coef_loc + ry*(2*1 + 1);
							alpha_vec_0 = svinsr(alpha_vec_0, alpha_loc[0]); 
							iv0_000 = svld1(mask_x_0, &ptr_src[(k_offset + rz)[0] + j_offset[ry] + (-1*16*4*4) + 0]); 
							iv0_001 = svld1(mask_x_1, &ptr_src[(k_offset + rz)[0] + j_offset[ry] + (-1*16*4*4) + (1*16*4*4)]); 
							iv0_002 = svld1(mask_x_2, &ptr_src[(k_offset + rz)[0] + j_offset[ry] + (-1*16*4*4) + (2*16*4*4)]); 
							spv000 = svext(iv0_000, iv0_001, 15); svmopa_za32_m(0, p32_all, p32_all, alpha_vec_0, spv000); 
							iv1_000 = svld1(mask_x_0, &ptr_src[(k_offset + rz)[1] + j_offset[ry] + (-1*16*4*4) + 0]); 
							iv1_001 = svld1(mask_x_1, &ptr_src[(k_offset + rz)[1] + j_offset[ry] + (-1*16*4*4) + (1*16*4*4)]); 
							iv1_002 = svld1(mask_x_2, &ptr_src[(k_offset + rz)[1] + j_offset[ry] + (-1*16*4*4) + (2*16*4*4)]); 
							spv100 = svext(iv1_000, iv1_001, 15); svmopa_za32_m(1, p32_all, p32_all, alpha_vec_0, spv100); 
							iv2_000 = svld1(mask_x_0, &ptr_src[(k_offset + rz)[2] + j_offset[ry] + (-1*16*4*4) + 0]); 
							iv2_001 = svld1(mask_x_1, &ptr_src[(k_offset + rz)[2] + j_offset[ry] + (-1*16*4*4) + (1*16*4*4)]); 
							iv2_002 = svld1(mask_x_2, &ptr_src[(k_offset + rz)[2] + j_offset[ry] + (-1*16*4*4) + (2*16*4*4)]); 
							spv200 = svext(iv2_000, iv2_001, 15); svmopa_za32_m(2, p32_all, p32_all, alpha_vec_0, spv200); 
							iv3_000 = svld1(mask_x_0, &ptr_src[(k_offset + rz)[3] + j_offset[ry] + (-1*16*4*4) + 0]); 
							iv3_001 = svld1(mask_x_1, &ptr_src[(k_offset + rz)[3] + j_offset[ry] + (-1*16*4*4) + (1*16*4*4)]); 
							iv3_002 = svld1(mask_x_2, &ptr_src[(k_offset + rz)[3] + j_offset[ry] + (-1*16*4*4) + (2*16*4*4)]); 
							spv300 = svext(iv3_000, iv3_001, 15); svmopa_za32_m(3, p32_all, p32_all, alpha_vec_0, spv300); 
							alpha_vec_1 = svinsr(alpha_vec_1, alpha_loc[1]); 
							spv000 = svext(iv0_001, iv0_002, 0); svmopa_za32_m(0, p32_all, p32_all, alpha_vec_1, spv000); 
							spv100 = svext(iv1_001, iv1_002, 0); svmopa_za32_m(1, p32_all, p32_all, alpha_vec_1, spv100); 
							spv200 = svext(iv2_001, iv2_002, 0); svmopa_za32_m(2, p32_all, p32_all, alpha_vec_1, spv200); 
							spv300 = svext(iv3_001, iv3_002, 0); svmopa_za32_m(3, p32_all, p32_all, alpha_vec_1, spv300); 
							alpha_vec_2 = svinsr(alpha_vec_2, alpha_loc[2]); 
							spv000 = svext(iv0_001, iv0_002, 1); svmopa_za32_m(0, p32_all, p32_all, alpha_vec_2, spv000); 
							spv100 = svext(iv1_001, iv1_002, 1); svmopa_za32_m(1, p32_all, p32_all, alpha_vec_2, spv100); 
							spv200 = svext(iv2_001, iv2_002, 1); svmopa_za32_m(2, p32_all, p32_all, alpha_vec_2, spv200); 
							spv300 = svext(iv3_001, iv3_002, 1); svmopa_za32_m(3, p32_all, p32_all, alpha_vec_2, spv300); 
						}
						for(int ry = 3; ry < 18; ry += 1) {
					...
					for(int rz = 0; rz < 3; rz += 1) {
						...
						for(int ry = 0; ry < 3; ry += 1) {
							float* alpha_loc = coef_loc + ry*(2*1 + 1);
							alpha_vec_0 = svinsr(alpha_vec_0, alpha_loc[0]); 
							mask_z = svdup_b32(0 < ker_size_z - vz + 1 - 0); 
							mask_y = svdup_b32(0 < ker_size_y - vy + 1 - (ry - 1)); mask_yz = svand_z(p32_all, mask_y, mask_z); mask_xyz = svand_z(p32_all, mask_yz, mask_x_0); iv0_000 = svld1(mask_xyz, &ptr_src[(k_offset + rz)[0] + j_offset[ry] + (-1*16*4*4) + 0]); mask_xyz = svand_z(p32_all, mask_yz, mask_x_1); iv0_001 = svld1(mask_xyz, &ptr_src[(k_offset + rz)[0] + j_offset[ry] + (-1*16*4*4) + (1*16*4*4)]); mask_xyz = svand_z(p32_all, mask_yz, mask_x_2); iv0_002 = svld1(mask_xyz, &ptr_src[(k_offset + rz)[0] + j_offset[ry] + (-1*16*4*4) + (2*16*4*4)]); 
							spv000 = svext(iv0_000, iv0_001, 15); svmopa_za32_m(0, p32_all, p32_all, alpha_vec_0, spv000); 
							mask_z = svdup_b32(0 < ker_size_z - vz + 1 - 1); 
							...
							spv100 = svext(iv1_000, iv1_001, 15); svmopa_za32_m(1, p32_all, p32_all, alpha_vec_0, spv100); 
							mask_z = svdup_b32(0 < ker_size_z - vz + 1 - 2); 
							mask_y = svdup_b32(0 < ker_size_y - vy + 1 - (ry - 1)); mask_yz = svand_z(p32_all, mask_y, mask_z); mask_xyz = svand_z(p32_all, mask_yz, mask_x_0); iv2_000 = svld1(mask_xyz, &ptr_src[(k_offset + rz)[2] + j_offset[ry] + (-1*16*4*4) + 0]); mask_xyz = svand_z(p32_all, mask_yz, mask_x_1); iv2_001 = svld1(mask_xyz, &ptr_src[(k_offset + rz)[2] + j_offset[ry] + (-1*16*4*4) + (1*16*4*4)]); mask_xyz = svand_z(p32_all, mask_yz, mask_x_2); iv2_002 = svld1(mask_xyz, &ptr_src[(k_offset + rz)[2] + j_offset[ry] + (-1*16*4*4) + (2*16*4*4)]); 
							spv200 = svext(iv2_000, iv2_001, 15); svmopa_za32_m(2, p32_all, p32_all, alpha_vec_0, spv200); 
							mask_z = svdup_b32(0 < ker_size_z - vz + 1 - 3); 
							mask_y = svdup_b32(0 < ker_size_y - vy + 1 - (ry - 1)); mask_yz = svand_z(p32_all, mask_y, mask_z); mask_xyz = svand_z(p32_all, mask_yz, mask_x_0); iv3_000 = svld1(mask_xyz, &ptr_src[(k_offset + rz)[3] + j_offset[ry] + (-1*16*4*4) + 0]); mask_xyz = svand_z(p32_all, mask_yz, mask_x_1); iv3_001 = svld1(mask_xyz, &ptr_src[(k_offset + rz)[3] + j_offset[ry] + (-1*16*4*4) + (1*16*4*4)]); mask_xyz = svand_z(p32_all, mask_yz, mask_x_2); iv3_002 = svld1(mask_xyz, &ptr_src[(k_offset + rz)[3] + j_offset[ry] + (-1*16*4*4) + (2*16*4*4)]); 
							spv300 = svext(iv3_000, iv3_001, 15); svmopa_za32_m(3, p32_all, p32_all, alpha_vec_0, spv300); 
							alpha_vec_1 = svinsr(alpha_vec_1, alpha_loc[1]); 
							spv000 = svext(iv0_001, iv0_002, 0); svmopa_za32_m(0, p32_all, p32_all, alpha_vec_1, spv000); 
							spv100 = svext(iv1_001, iv1_002, 0); svmopa_za32_m(1, p32_all, p32_all, alpha_vec_1, spv100); 
